...
Versión corregida con sintaxis correcta para PyOxidizer y plantilla final.
"""
...
'''
    ...
    # --------------------------------------------------------------------------
    # Builder: PyOxidizer (con plantilla final)
    # --------------------------------------------------------------------------
    ...
    def _create_pyoxidizer_config(self, target_platform):
        """Genera pyoxidizer.bzl usando la plantilla final."""
        icon_path = self.configs[target_platform]["icon"]
        ext = self.configs[target_platform]["ext"]
        exe_name = f"soundvi{ext}"
        
        # Contenido basado en pyoxidizer_soundvi_final.bzl
        config = f'''# pyoxidizer.bzl para Soundvi - Generador de Video con Visualizador
...
'''
        with open(self.project_dir / "pyoxidizer.bzl", "w") as f:
            f.write(config)
        print("[PyOxidizer] Archivo de configuración pyoxidizer.bzl generado con plantilla final.")
