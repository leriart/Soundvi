...
_USER_PREFS_FILE = os.path.join(_RAIZ, "user_preferences.json")
...
def cargar_preferencias() -> Dict[str, Any]:
    """Carga user_preferences.json. Retorna dict vacio si no existe."""
    if not os.path.isfile(_USER_PREFS_FILE):
        return {}
    try:
...
def guardar_preferencias(prefs: Dict[str, Any]):
    """Guarda preferencias en user_preferences.json."""
    try:
...
def es_primer_inicio() -> bool:
    """Retorna True si no existe user_preferences.json (primer inicio)."""
    return not os.path.isfile(_USER_PREFS_FILE)
...
class SelectorTema(QFrame):
    ...
    def _construir_ui(self):
        ...
        # Boton tema oscuro
        self._btn_oscuro = QPushButton("🌙  Oscuro")
        ...
        # Boton tema claro
        self._btn_claro = QPushButton("☀️  Claro")
...
class MenuQueQuieresHacer(QDialog):
    ...
    def _crear_paso_tema(self) -> QWidget:
        ...
        self._card_oscuro = self._crear_tarjeta_tema(
            "darkly", "🌙  Tema Oscuro",
            "Colores oscuros que reducen la fatiga visual.\nIdeal para sesiones largas de edición.",
            "#212529", "#DEE2E6", "#375A7F"
        )
        ...
        self._card_claro = self._crear_tarjeta_tema(
            "claro", "☀️  Tema Claro",
            "Colores claros para ambientes bien iluminados.\nAlto contraste y legibilidad.",
    ...
    def _seleccionar_tema(self, clave: str):
        ...
        self._actualizar_estilo_tema(self._card_oscuro, clave == "darkly")
        self._actualizar_estilo_tema(self._card_claro, clave == "claro")
    ...
    def _actualizar_estilo_tema(self, frame: QFrame, activo: bool):
        ...
        acento = "#375A7F" if clave == "darkly" else "#0D6EFD"
        if activo:
